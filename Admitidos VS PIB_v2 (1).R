...
# AQUÍ ES DONDE CAMBIAS - Limpieza y conversión de la columna SEMESTER
data <- data %>%
  mutate(
    SEMESTER = gsub("[^0-9]", "", SEMESTER), # Eliminar caracteres no numéricos
    SEMESTER = as.numeric(SEMESTER),         # Convertir a numérico
    SEMESTER = SEMESTER / 10                 # NUEVA LÍNEA: Dividir por 10 para quitar el 0
  )

print(head(data))

# ALTERNATIVA 1: Si prefieres hacerlo en un solo paso
data <- data %>%
   mutate(
     SEMESTER = gsub("[^0-9]", "", SEMESTER), # Eliminar caracteres no numéricos
     SEMESTER = as.numeric(substr(SEMESTER, 1, 4)) # Tomar solo los primeros 4 dígitos
   )
...
ggplot(data, aes(x = SEMESTER, y = COUNT, fill = GENDER)) +
  ...
  labs(
    title = "Gender Distribution by Academic Semester",
    x = "Academic Semester",
...
ggplot(data_total, aes(x = SEMESTER, y = Total_Admitidos)) +
  ...
  geom_point() +
  labs(
    title = "Total Admission Trend by Academic Semester",
    x = "Academic Semester",
    y = "Total Number of Admitted"
  ) +
  theme_minimal()
...
ggplot(data, aes(x = SEMESTER, y = COUNT, color = GENDER)) +
  ...
  geom_point() +
  labs(
    title = "Admission Trends by Gender and Academic Semester",
    x = "Academic Semester",
    y = "Number of Admitted",
    color = "Gender"
  ) +
